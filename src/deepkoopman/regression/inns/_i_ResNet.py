...
class LinearIResNetBlock(InvertibleModule):
    def __init__(self, dims_in, dims_c=[],
        ...
        in_dim = dims_in[0][0]
        layers = [spectral_norm_fc(nn.Linear(in_dim, internal_dim), coeff)]
        for _ in range(n_layers):
            layers.append(spectral_norm_fc(nn.Linear(internal_dim, internal_dim), coeff))
        layers.append(spectral_norm_fc(nn.Linear(internal_dim, in_dim), coeff))


        activations = [nn.ELU() for _ in layers]
        module_list = sum(zip(layers, activations), ())[:-1]
        self.residual = nn.Sequential(*module_list)

    def forward(self, x, c=[], rev=False, jac=True):
        if not rev:
            return [x[0] + self.residual(x[0])], None
        else:
            y = x[0]
            x_hat = y
            for _ in range(36):  # fixed point iterations
...
class iResNetFreia(BaseINN):
    ...
    def __init__(self,
        ...
        # Graph definieren
        nodes = [Ff.InputNode(self.input_size, name='input')]
        ...
        self.model = Ff.GraphINN(nodes, verbose=False, force_tuple_output=False)

    def _process_2d(self, x, rev=False):
        if not rev:
            out, _ = self.model(x, jac=False)
            return out
        else:
            out, _ = self.model(x, rev=True, jac=False)
            return out
    ...
    def forward(self, x):
        if x.dim() == 2:
            ...
            return out_2d.reshape(b, t, -1)
        else:
            raise ValueError("Input muss 2D oder 3D sein.")
